...
function parse_arr($field){
			...
			if ($itempart1 == 'SUBFIELD'){
				...
				}	elseif (is_array($item)) $item_arr[] = $this->parse_arr($item);
				else $item_arr[] = $item;
			}
			return $item_arr;
			}	
function init($record){
	...
	if ($part1 == 'FIELD' && is_array($field)){
		...
		if ($itempart1 == 'SUBFIELD'){
				$item_arr[$itempart2] = $item;
			} elseif (is_array($item)) $fields[$part2.'_'.$itemkey] = $this->parse_arr($item);
		}
		$fields[$part2] = $item_arr;
		} else $fields[$part2] = $field;
	}	
	return $fields;
}
...
function getRecordName(){
	if ($this->validate()) return $this->record['200']['A'];
		return false;
}


function xmlInit(){
	if (isset($this->record['700']['A'])) $this->xmlData[] = array(
		'attr'=>array('element' => 'contributor',
		'qualifier' => 'author'),
		'value' =>$this->record['700']['A'].', '.$this->record['700']['B']
		);
	if (isset($this->record['701']['A'])) $this->xmlData[] = array(
		'attr'=>array('element' => 'contributor',
		'qualifier' => 'author'),
	...
	if (isset($this->record['463']['J'])) $this->xmlData[] = array(
	...
	if (isset($this->record['963']['J'])) $this->xmlData[] = array(
				'attr'=>array('element' => 'publisher',
		 'language'=>"ru_RU"),
		'value' =>$this->record['963']['J']
		);
	if (isset($this->record['200']['A'])) $this->xmlData[] = array(
		'attr'=>array(	
		'element' => 'title',
		 'language'=>"ru_RU"),
	...
	if (isset($this->record['463']['G'])) $this->xmlData[] = array(
		'attr'=>array(	
		'element' => 'publisher',
		 'language'=>"ru_RU"),
...
';
  ...
  $string .= '  <dcvalue ';
   foreach ($val['attr'] as $attrName=>$attr) $string .= ' '.$attrName.'="'.$attr.'"';
...
function validate(){
	if (!isset($this->record['200']['A'])) {
		...
		echo '<pre>';print_r($this->record);echo '</pre>';
		return false;
		}
	foreach ($this->required_fields as $key=>$fields) foreach ($fields as $subkey){
			if (!isset($this->record[$key][$subkey])){
				echo 'Помилка! <br/>У записі з заголовком <u>'.$this->record['200']['A'].'</u><br/>відсутне поле поле <b>'.$key.'-'.$subkey.'</b><br />';
				return false;
				}
		}
	return true;	
...
function createSrc($dirname){
	if (!$dirname && $this->validate()) return false; 
            ...
            if(!isset($current[$tag])) { //New Key
                ...
                if(isset($current[$tag][0]) and is_array($current[$tag])) {//If it is already an array...
                    ...
                    if($priority == 'tag' and $get_attributes) {
                        if(is_array($current) && isset($current[$tag.'_attr'])) { //The attribute of the last(0th) tag must be moved as well
                            $current[$tag]['0_attr'] = $current[$tag.'_attr'];
                            unset($current[$tag.'_attr']);
                        }
